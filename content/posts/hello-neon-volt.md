...
It's a portable smart power supply that runs off of USB-C, utilises the rev 2 power
delivery spec and is configurable over UART, but also comes with physical buttons,  so you can use it 
without a computer. At the time of me writing this, it's still in the system design phase, but I do have a preliminary spec for it and that goes as follows: 
100W total power, 98.5% efficient at 20V input, 10mV adjustment
steps, controllable via PC command line (so it's scriptable) or via the knobs and keys on the device. I will not be able to guarantee that efficiency and the converter will probably not be
that precise, especially with cable drop at higher currents. I can't foresee everything that will go wrong with it, but I'll do my best to make it work as best I can. 
...
user addressable terminal block, a user accessible ADC and DAC pins, a dedicated 3V3 rail and an adjustable output of 
1 to 48 volts, a color 320x240 screen and enough flash memory to comfortably run Doom on this thing. 
...
waiting on system access or just openings in other people's calendars. This had me thinking of maybe picking up a project 
to work on, so my skills don't atrophy. My pick ended up being a SCARA design from a few years back, 
...
make for a fun commute. I couldn't bring any of my gear early on as during my mandatory trial period, I lived in hostels and AirBnB-s. 
For 4 months. This is not favorable to lab type of work. The worst was that out of all the things, I lacked a convenient portable.
power source. This lit a lightbulb - why not make one? I lack practice in power electronics so I could get hands on experience. I can customise it. 
I can play with the USB-C power delivery specification. 
Instead of doing it right, I dove headfirst into the more fun parts like selecting the switcher, mapping the transfer function and drawing the Bode plot etc, but I figured, before I commit too much of my time to this, I should at least CHECK
what's on the market fist. The results were not inspiring. I dislike the toaster looking devices because they're bulky, dumb and don't do USB. But they are cheap so there's that. 
Then there's the two USB based chinese units, FNIRSI DPS-150 (really rolls off the tongue, that one) and AlienTek DP100.
There's also the [PocketPD project.]({{< relref "https://hackaday.io/project/194295-pocketpd-usb-c-portable-bench-power-supply" >}})
...
The chinese units start at 5V while the pocketPD starts at 3V3. This is good for a lot of hobbyist stuff, so for many people, these will be okay. The pocketPD irks me though. It has no switcher and as such it has none of the expensive parts, they're using the cheap as hell small OLEDs and also making them in China. And somehow they're still charging $60 per unit. The idea is there though - a type C power brick already has a switcher, so why add another. 
Well, I'm making a _smart_ power supply here. I need it to put out volts, yes, but also I want to program custom sweeps, ADC or GPIO triggered voltage/current events, etc. This is not something you can get from any of these units. Sure, I could buy a Keithley or a R&S, but I like having both of my kidneys. And those units won't be portable, nor USB friendly. And then there's the issue of playing Doom on it. No Keithley runs Doom and dammit this is important. 
...
Ah, yeah. Welcome to this thing I started. I want to share some knowledge and lower the barrier of entry for electronics. I'm not really sure where this will go, but I figured 
the best starting point is to document my builds and projects. As for the naming, I'm pretty big on the whole cyberpunk aesthetic, so that's neon. I play with robots, so Golem - an automaton from the jewish mythos.
